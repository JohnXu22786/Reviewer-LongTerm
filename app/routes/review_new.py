...
This extended version follows the core principle: for files that exist
...
def reset_review_state():
    ...
    try:
        ...
        # Also reset plugin long-term storage if available
        if PLUGIN_AVAILABLE and call_plugin_func:
            try:
                plugin_result = call_plugin_func(
                    "learning_reviewer",
                ...
                # Plugin errors should not affect the main reset logic
                import logging
                logging.getLogger(__name__).warning(f"Plugin reset call failed: {e}")
